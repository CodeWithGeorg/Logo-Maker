      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16 w-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {mode === 'create' && (
                    <div className="flex gap-2">
                      <button onClick={() => handlePromptChange(activeState.prompt + " Minimalist")} className={`text-[10px] px-2 py-1 rounded transition-colors ${darkMode ? 'bg-slate-800 text-slate-400 hover:bg-indigo-900/50 hover:text-indigo-400' : 'bg-slate-50 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600'}`}>#Minimalist</button>
                      <button onClick={() => handlePromptChange(activeState.prompt + " Modern")} className={`text-[10px] px-2 py-1 rounded transition-colors ${darkMode ? 'bg-slate-800 text-slate-400 hover:bg-indigo-900/50 hover:text-indigo-400' : 'bg-slate-50 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600'}`}>#Modern</button>
                    </div>
                  )}
                </div>
                <textarea
          ...
          <div id="result-preview">
            <div className={`p-6 md:p-10 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border flex flex-col h-full min-h-[500px] transition-all ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
              <div className="flex items-center justify-between mb-6">
                ...
                {activeState.result && (
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter ${darkMode ? 'bg-emerald-900/40 text-emerald-400' : 'bg-emerald-50 text-emerald-500'}`}>Rendered 4K</span>
                )}
              </div>
              ...
              <div className={`flex-grow flex items-center justify-center rounded-3xl border-2 border-dashed overflow-hidden relative group transition-all ${darkMode ? 'bg-slate-800/20 border-slate-700' : 'bg-slate-50/50 border-slate-200/60'}`}>
                  ...
                  <div className={`absolute inset-0 z-10 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500 ${darkMode ? 'bg-slate-900/95' : 'bg-white/95'}`}>
                    <div className="relative mb-6">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <i className={`fas fa-brain text-xl animate-pulse ${darkMode ? 'text-indigo-400/30' : 'text-indigo-600/30'}`}></i>
                      </div>
                    </div>
                ...
                {activeState.result ? (
                  ...
                  <div className="text-center p-12 opacity-40">
                    ...
                    <p className={`font-bold text-lg mb-2 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Awaiting Strategy</p>
        ...
        <section id="process" className="py-20 md:py-32 scroll-mt-20">
          <div className="text-center mb-16">
            <h2 className={`text-3xl md:text-5xl font-black mb-4 tracking-tighter ${darkMode ? 'text-white' : 'text-slate-900'}`}>The Master Studio Process</h2>
            <p className="text-slate-400 font-medium max-w-xl mx-auto">How we turn concepts into world-class brand identities.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: "fa-fingerprint", title: "Identity Analysis", desc: "Our AI breaks down your core visual DNA, understanding every curve and color intention." },
              { icon: "fa-vector-square", title: "Vector Synthesis", desc: "Complex algorithms recreate your mark with mathematical precision for infinite scalability." },
              { icon: "fa-gem", title: "Final Polishing", desc: "Humanized brand logic applies professional finishes, typography, and premium aesthetics." }
            ].map((step, i) => (
        ...
        {history.length > 0 && (
          <div id="gallery" className="mt-20 md:mt-32 animate-in fade-in slide-in-from-bottom-8 duration-1000 scroll-mt-24">
            <div className="flex items-center justify-between mb-10">
              <h2 className={`text-2xl md:text-3xl font-black flex items-center gap-3 tracking-tighter ${darkMode ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            ...
            <div className="flex justify-center md:justify-end gap-6 text-slate-400">
              <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Twitter...'); }} className={`w-10 h-10 rounded-full border flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}><i className="fab fa-twitter text-sm"></i></a>
              <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Dribbble...'); }} className={`w-10 h-10 rounded-full border flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}><i className="fab fa-dribbble text-sm"></i></a>
              <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to LinkedIn...'); }} className={`w-10 h-10 rounded-full border flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}><i className="fab fa-linkedin-in text-sm"></i></a>
            </div>
          </div>
